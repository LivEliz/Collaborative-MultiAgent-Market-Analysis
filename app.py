...
from main import collaborative_analysis
...
def keyword_chart(keywords):
    if not keywords:
        return None
    # Convert keywords list into a dataframe
    df_kw = pd.DataFrame(keywords, columns=["keyword"])
    df_kw["count"] = df_kw["keyword"].map(df_kw["keyword"].value_counts())
    # Build bar chart
    fig = px.bar(
        df_kw.drop_duplicates("keyword"),
        x="keyword",
        y="count",
    ...
    if not query.strip():
        ...
        with st.spinner("Running multi-agent analysis..."):
            ...
            if demo_mode:
                ...
                try:
                    st.session_state.result = collaborative_analysis(query)
...
if st.session_state.result and st.session_state.insights:
    ...
    with tabs[1]:
        ...
        # Show keyword chart
        chart = ins.get("charts", {}).get("keywords")
        if chart:
            st.plotly_chart(chart, use_container_width=True)
        else:
            # fallback: build chart from market_trends keywords
            fig = keyword_chart(trends)
            if fig:
    ...
    # TAB 3 — Sentiment
    with tabs[2]:
        if ins.get("charts", {}).get("sentiment"):
            st.plotly_chart(ins["charts"]["sentiment"], use_container_width=True)
    ...
    with tabs[5]:
        if df.empty:
            ...
            st.dataframe(view_df[columns].head(50), use_container_width=True)
